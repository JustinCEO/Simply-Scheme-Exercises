...
(define (match-using-known-values pattern sent known-values)
  (cond ((empty? pattern)
	 (if (empty? sent) (reverse known-values) 'failed)) ; added a reverse
...
;many changes here for problem 17.15
...
(define (add name value known-values)
  (cond ((empty? name) known-values)
      (else (cons (list name value) known-values))))
...
(define testlist '((front (your mother))(back (should know))))

;(match '(!front !middle *back) '(your mother should know))
;'((front (your)) (middle (mother)) (back (should know)))
